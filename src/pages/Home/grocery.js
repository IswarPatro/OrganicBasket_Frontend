...
const grocery = () => {
    ...
    const transformProducts = () => {
        ...
        if (sort==="price2") {
        sortedProducts = sortedProducts.filter((prod) => prod.price>=50 && prod.price<=99);
        }
        if (sort==="price3") {
        sortedProducts = sortedProducts.filter((prod) =>  prod.price>=100 && prod.price<=149);
        }
        if (sort==="price4") {
        sortedProducts = sortedProducts.filter((prod) =>  prod.price>=150 && prod.price<=199);
        }
      ...
      return (
        ...
        <h1 className="text-secondary mt-4">Grains</h1>
        <div className="row row-cols-1 row-cols-md-4 g-3 gy-1 my-2  d-flex justify-content-center  ">
            {
                ...
                <img src={item.img} class="card-img-top m-auto mt-2"
                        alt="Skyscrapers" style={{width:200,height:130}}/>
                    <div className="card-body">
                        <h5 className="card-text text-secondary">Tata</h5>
